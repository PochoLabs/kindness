...
function cmsmasters_divider_top() {
    return '<div class="divider">' . 
        '<a class="fr" href="#"><small>' . __('Top', 'cmsmasters') . ' &uarr;</small></a>' . 
        '<div class="cl"></div>' . 
    '</div>';
}
    ...
    if ($posttype_query->have_posts()) :
        while ($posttype_query->have_posts()) : $posttype_query->the_post();
					...
					if (has_post_thumbnail()) {
						...
						foreach ($attachments as $attachment) {
							if (!isset($counter) && $counter = true) {
								$out .= cmsms_thumb(get_the_ID(), 'project-thumb', true, false, true, false, true, false, true, $attachment->ID, true);
							}
						}
					} else {
			...
			if ($type != 'aside' && $type != 'link' && $type != 'quote') {
				...
				$out .= '<header class="entry-header">' . 
					'<h5 class="entry-title">' . theme_excerpt(20, false) . '</h5>' . 
				'</header>';
			} elseif ($type == 'quote') {
				$out .= '<header class="entry-header">';
			...
			if ($read_more == 'true' && $type != 'aside' && $type != 'link' && $type != 'quote') {
				...
				$out .= '<a class="button" href="' . get_permalink(get_the_ID()) . '"><span>' . $post_more_text . '</span></a>';
			}
...
function cmsmasters_slider($atts, $content = null) {
    ...
    foreach ($sliderOptions['slider']['slides'] as $slide) {
        ...
        if ($show_caption == 'true') {
            ...
            if ($caption_subtitle != '') {
                $out .= '<h2>' . $caption_subtitle . '</h2>';
            }
			
            if ($caption_text != '') {
				if (substr($caption_text, 0, 1) == '<' || substr($caption_text, -1) == '>') { 
					$out .= $caption_text;
				} else { 
